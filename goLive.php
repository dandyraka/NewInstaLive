    ...
    try {
        ...
        if (!Utils::isRecovery()) {
            if(dandyMode){
                ...
                if(filter_var($vidInput, FILTER_VALIDATE_URL)) {
                    $isyt = preg_match('/(youtube.com|youtu.be)/', $vidInput);
                    if($isyt){
                        Utils::log("Getting video from youtube...");
                        $videoFilename = getYTvid($vidInput);
                    } else {
                        Utils::log("Make sure you input the correct video url.\nGetting video...");
                        $videoFilename = $vidInput;
                    }
                } else {
                    if (file_exists($vidInput)) {
                        Utils::log("Getting video from file...");
                        $videoFilename = $vidInput;
                    } else {
                        Utils::log("The file ". $vidInput." does not exist.");
                        exit();
                    }
                }
                $ffmpegPath = null;
...
function getSnippet($videoID) {
    $query = file_get_contents('https://www.googleapis.com/youtube/v3/videos?id=' . $videoID . '&key=AIzaSyDcwbYdOm9CUHnDjm7dma0yaCDtzHPHE40&part=snippet');
    $query = json_decode($query);
    if (empty($query->items)) {
        return null;
    }
    $query = $query->items[0]->snippet;
    return [
        'id'          => $videoID,
        'title'       => $query->title,
        'thumbnail'   => $query->thumbnails->default->url
    ];
}

function getVideo($video) {
    $videosByQuality = array_map(function($vid) {
        if (strpos($vid->mimeType, 'video') !== false) {
            return [
                'bitrate' => $vid->bitrate,
                'url'     => $vid->url,
                'type'    => $vid->mimeType,
                'cipher'  => $vid->cipher ?? null
            ];
        }
    }, $video);

    $videosByQuality = array_filter($videosByQuality);

    usort($videosByQuality, function($vidA, $vidB) {
        return $vidB['bitrate'] > $vidA['bitrate']; 
    });

    $assets = $videosByQuality[0];
    if (!empty($assets['url'])) {
        return [
            'url'  => $assets['url'],
            'type' => $assets['type']
    ...
    if (!empty($assets['cipher'])) {
        parse_str($assets['cipher'], $cipher);
        return [
            'url'  => $cipher['url'],
            'type' => $assets['type']
        ];
...
function getAudio($video) {
    ...
    foreach ($video as $vid) {
        if (strpos($vid->mimeType, 'audio') !== false) {
            $assets = [
                'url'    => $vid->url,
                'type'   => $vid->mimeType,
                'cipher' => $vid->cipher ?? null
...
function getVideoExpire($url) {
    $url = parse_url($url);
    parse_str($url['query'], $queryString);
    return date('Y-m-d H:i:s', $queryString['expire']);
}

function getVideoInfo($videoID) {
    $query = file_get_contents('https://www.youtube.com/get_video_info?video_id=' . $videoID);
    parse_str($query, $queryArgs);

    // Get video info by streaming data
    if (isset($queryArgs['player_response'])) {
        $stream = json_decode($queryArgs['player_response']);
        if (property_exists($stream, 'microformat') && property_exists($stream, 'streamingData')) {
            $video = $stream->streamingData->adaptiveFormats;
            if (empty($video)) {
                return null;
            }
            ...
            $videoData = [
                'id'        => $videoID,
                'title'     => $stream->microformat->playerMicroformatRenderer->title->simpleText,
                'thumbnail' => $stream->microformat->playerMicroformatRenderer->thumbnail->thumbnails[0]->url,
                'video'     => getVideo($video),
                'audio'     => getAudio($video),
            ];
            $videoData['expired_at'] = getVideoExpire($videoData['video']['url']);
            return $videoData;
        }
    }

    // get video by url encoded stream map
    if (!isset($queryArgs['url_encoded_fmt_stream_map'])) {
        return null;  
    }
    parse_str($queryArgs['url_encoded_fmt_stream_map'], $stream);
    return array_merge(getSnippet($videoID), [
        'url'        => $stream['url'],
        'expired_at' => date('Y-m-d H:i:s', time() + 21599),
    ]);
}
